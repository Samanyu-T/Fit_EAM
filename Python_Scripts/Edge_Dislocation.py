...
def create_dislocation():
    ...
    lmp.command('region r_simbox block %f %f %f %f %f %f units lattice' % (
        ...
        -1e-9, size + 1e-9, -1e-9 - 10, size + 1e-9 + 10, -1e-9 , size + 1e-9 
    ))

    lmp.command('region r_atombox1 block %f %f %f %f %f %f units lattice' % (

        -1e-4, size + 1e-4, -1e-4, size//2, -1e-4, size + 1e-4
    ))


    lmp.command('region r_atombox2 block %f %f %f %f %f %f units lattice' % (

        -1e-4, size - 1 + 1e-4 , size//2 + 1e-4, size + 1e-4, -1e-4, size + 1e-4
    ))
    ...
    idx = np.where( atom_pos[:, -2] > z_lat*(size//4 + 0.01))[0]
...
def binding(potfile):
    ...
    for i, _z in enumerate(z):
        ...
        lmp.command('read_data Lammps_Dump/Dislocations/Edge/Edge_New.data')
