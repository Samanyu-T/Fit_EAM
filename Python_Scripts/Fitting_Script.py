...
for iteration in range(N):
    ...
    with open('Optimization_Files/Sample_Files/samples_%d.txt' % iteration, 'w') as file:
        file.write('Start Optimization \n')

    with open('Optimization_Files/Loss_Files/loss_%d.txt' % iteration, 'w') as file:
        file.write('Start Optimization \n')

    x_init = fitting_class.init_sample(isrand=True)
    x_star = minimize(loss_func, args=(fitting_class, pot, ref_dict, iteration), x0=x_init, method = 'COBYLA')
